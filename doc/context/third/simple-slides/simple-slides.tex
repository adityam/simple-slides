...
\usemodule[int-load]
% \usemodule[abr-01]
\def\MKII{\smallcapped{mkii}}
\def\MKIV{\smallcapped{mkiv}}
...
\definefont [titlefont]    [LMSans-DemiCondensed]
\setuphead[title][alternative=middle, textstyle=\titlefont] 
\setuphead[section,subsubject][numberstyle=\titlefont,textstyle=\titlefont]

\useURL[practex][http://www.tug.org/pracjourn/2006-2/schmitz/]

\defineframedtext[EXAMPLE]
                 [width=\textwidth,background=color,backgroundcolor=gray]
...
\filename{t-simpleslides-<date>.zip} into one of your \filename{$TEXMF} tree,
...
\stopitemize
To use a style, say \options{BigNumber}, pass \options{style=BigNumber} option
...
Some of the styles come have some variants, which will be explained later. The
output of different styles is shown in \in Section[sec:styles].
...
The macro \typeTEX{\placeTitle} places the title page in the presentation.
...
progress bars, and set up the background. The content of the slides follows
after this command.
...
\placefigure
  ...
  \startcombination[2*2]
    \startEXAMPLE[width=0.55\textwidth]
\startTEX
  \usemodule[simpleslides]
            [style=BigNumber]
  \setupTitle[...]
