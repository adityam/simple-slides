...
placement of pictures. It takes four arguments (one of which is options, and as
...
placed in a horizontal or vertical arrangement; fir examples, see \in
Figure[fig:pictures]. The second argument is the filename of the picutre that
...
The third argument is the most complex. It specifies picutre dimensions and
...
\startitemize[packed]
  ...
  \item \options{highlight} \par
    This key determines highlighting of the picture. If you set,
    \options{hightlight=yes}, then you can use one of the three available
  ...
  \item \options{alternative} \par
    When \options{highlight=yes}, three different forms of highlights are
...
\placefigure
  [top,bottom]
  [fig:highlight]
  {Differnt highlight options available}
  \startcombination[2*2]
...
you do not have to change the location of your hightlights if you change the
...
\startitemize[packed]
  ...
  \item \options{gridcolor} \par
    This option determines the color in which the grid is drawn. It can talk and
...
ratated by that amount (in degrees) in the counter clockwise direction. The
...
the picture. The area to be focuses is a circle (or an ellipse) and it can be
specified using \options{x} and \options{y} to indicate the center,
\options{xscale} and \options{yscale} to indicate the raidius, and
\options{direction} to indicate the ratation. The keys \options{rulethickness}
